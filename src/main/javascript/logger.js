(function() {
    ...
    var simpleAjaxRequest = function() {
    ...
    var logger = function (config) {
        var serverURL = 'http://localhost:1337/json',
            errorsBatch = [],
            interval = config && config.interval ? config.interval : 5000;

        function postErrorCache() {
            if (errorsBatch.length > 0) {
                simpleAjaxRequest.post({
                    url : serverURL,
        ...
        return {
            log : function(msg, url, line, details) {
                var error = {
                    'message' : msg,
                    'url' : url,
                    'lineNumber' : line,
